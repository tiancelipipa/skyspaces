...
Vector2R BacktracePosition2D(
    ...
    switch (integrator) {
        case AdvectionIntegrator2D::ExplicitEuler: {
            const Vector2R k1 = velocity_at(position);
            return position - dt * k1;
        }
        case AdvectionIntegrator2D::Midpoint: {
            const Vector2R k1 = velocity_at(position);
            const Vector2R midpoint = position - 0.5 * dt * k1;
            const Vector2R k2 = velocity_at(midpoint);
            return position - dt * k2;
        }
        case AdvectionIntegrator2D::RungeKutta3: {
            const Vector2R k1 = -velocity_at(position);
            const Vector2R k2 = -velocity_at(position + 0.5 * dt * k1);
            const Vector2R k3 = -velocity_at(position + 0.75 * dt * k2);
            return position + dt * ((2.0 / 9.0) * k1 + (3.0 / 9.0) * k2 + (4.0 / 9.0) * k3);
        }
        case AdvectionIntegrator2D::RungeKutta4: {
            const Vector2R k1 = -velocity_at(position);
            const Vector2R k2 = -velocity_at(position + 0.5 * dt * k1);
            const Vector2R k3 = -velocity_at(position + 0.5 * dt * k2);
            const Vector2R k4 = -velocity_at(position + dt * k3);
